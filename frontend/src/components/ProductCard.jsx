...
export default function ProductCard({ product, onAdd, onEdit }) {
  ...
  return (
    <button
      onClick={onAdd}
      className="group relative bg-gradient-to-br from-white/95 to-purple-50/95 rounded-2xl shadow-lg overflow-hidden border border-white/30 hover:shadow-2xl hover:scale-[1.02] transition-all duration-300 w-full text-left focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-opacity-50"
    >
      {/* Edit Button - positioned absolutely */}
      <button
        onClick={handleEditClick}
        className="absolute top-2 left-2 z-10 p-2 rounded-lg bg-gradient-to-r from-blue-500 to-cyan-500 text-white hover:shadow-lg hover:scale-110 active:scale-95 transition-all duration-200"
        title="Edit Product"
      >
